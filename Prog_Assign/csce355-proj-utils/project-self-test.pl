...
$test_files_root = "$ENV{HOME}/Documents/CSCE355/Prog_Assign/test";

# Directory that contains the utilities in2post and pre2in
# (an absolute pathname so it need not be a subdirectory
# of the submission root)
$utils = "$ENV{HOME}/Documents/CSCE355/Prog_Assign/csce355-proj-utils/bin";
...
$default_submission_root = "$ENV{HOME}/Documents/CSCE355/Prog_Assign/csce355-proj-utils/src";
...
%command_line_args_hash = (
    ...
    'prefixes'       => 'transform',
    'suffixes'       => 'transform',
    'b-before-a'     => 'transform',
    'drop-one'       => 'transform',
    'strip'          => 'transform symbol',
#    'omit'           => 'experimental transform symbol',
#    'simplify'       => 'experimental transform'
);
...
# Keys are tasks successfully implemented;
...
sub report_summary {
    ...
    foreach $arg (@command_line_args_array) {
	$point_value = ($arg eq 'no-op' ? 40 : 5)*$progress{$arg};
